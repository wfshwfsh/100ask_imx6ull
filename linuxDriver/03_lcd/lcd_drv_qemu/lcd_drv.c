...
int __init imx6ull_fb_init(void)
{
	int ret;
    dma_addr_t phy_addr;
    ...
    /* fb的虚拟地址 */
    unsigned map_size = PAGE_ALIGN(myfb_info->fix.smem_len);
...
static void __exit imx6ull_fb_exit(void)
{
	/* unregister */
...
module_init(imx6ull_fb_init);
module_exit(imx6ull_fb_exit);
