...
#else
	printk("[%s] set pin %d as output %s\n", __func__, offset, val ? "high" : "low");
	g_gpio_dir &= ~(1<<offset);
#endif
	return 0;
}
...
static int virtual_gpio_probe(struct platform_device *pdev)
{
	...
	//g_virt_gpio->label   = 
	g_virt_gpio->base    = -1;//determin by system
    unsigned int ngpio;
	ret = of_property_read_u32(pdev->dev.of_node, "ngpios", &ngpio);
    g_virt_gpio->ngpio   = ngpio;
