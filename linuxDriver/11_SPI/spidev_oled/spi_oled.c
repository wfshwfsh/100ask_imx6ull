...
void dc_pin_init(int number)
{
    ...
    snprintf(cmd, sizeof(cmd), "echo %d > /sys/class/gpio/export", number);
    system(cmd);
    
    snprintf(cmd, sizeof(cmd), "echo out > /sys/class/gpio/gpio%d/direction", number);
    system(cmd);
}
...
void dc_pin_exit(int number)
{
	...
	char cmd[100];
    snprintf(cmd, sizeof(cmd), "echo %d > /sys/class/gpio/unexport", number);
    system(cmd);
}

/* DC=0: CMD, DC=1: DATA*/
void oled_set_dc_pin(int dc)
{
    char cmd[100];
    int val;
    ...
    snprintf(cmd, sizeof(cmd), "echo %d > /sys/class/gpio/gpio%d/value", val, dc_pin_num);
    system(cmd);
}
